...
// Fonction pour animer les éléments d'entrée
function animateEntryElements() {
    ...
    statCards.forEach((card, index) => {
        card.classList.add('glass-card', 'zoom-in');
    ...
    // Animer les cartes de contenu
    const contentCards = document.querySelectorAll('.modern-card');
...
function updateStats() {
    ...
    const totalBeneficiaries = projects.reduce((sum, project) => sum + (project.target_beneficiaries || 0), 0);
    const totalAffected = problems.reduce((sum, problem) => sum + (problem.affected_population || 0), 0);
...
function animateCounter(elementId, targetValue) {
    ...
    // Utiliser l'animation de compteur avancée si disponible
    if (window.animationManager) {
        window.animationManager.animateCounter(element, targetValue, 2000);
    } else {
        // Fallback vers l'animation simple
        let currentValue = 0;
...
function renderProblems(filteredProblems = null) {
    ...
    problemsGrid.innerHTML = problemsToRender.map(problem => `
        <div class="bg-white rounded-lg shadow-md p-6 card-hover problem-card cursor-pointer floating-card slide-up" onclick="showProblemDetail(${problem.id})" style="animation-delay: ${Math.random() * 0.5}s;">
            ...
            <div class="space-y-2 text-sm text-gray-500">
                ...
                <div class="flex items-center">
                    ...
                    <span>${(problem.affected_population || 0).toLocaleString()} personnes affectées</span>
                </div>
...
function renderProjects(filteredProjects = null) {
    ...
    projectsGrid.innerHTML = projectsToRender.map(project => `
        <div class="bg-white rounded-lg shadow-md p-6 card-hover project-card cursor-pointer floating-card slide-up" onclick="showProjectDetail(${project.id})" style="animation-delay: ${Math.random() * 0.5}s;">
            <div class="flex justify-between items-start mb-4">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="text-center p-3 bg-blue-50 rounded-lg">
                    <div class="text-xl font-bold text-blue-600">$${(project.required_budget || 0).toLocaleString()}</div>
            ...
            <div class="space-y-2 text-sm text-gray-500">
                <div class="flex items-center">
                    ...
                    <span class="ml-auto">
                        ...
                        ${(project.target_beneficiaries || 0).toLocaleString()} bénéficiaires
                    </span>
                </div>
...
function renderInvestments(filteredInvestments = null) {
    ...
    investmentsGrid.innerHTML = investmentsToRender.map(investment => `
        <div class="bg-white rounded-lg shadow-md p-6 card-hover investment-card cursor-pointer floating-card slide-up" onclick="showInvestmentDetail(${investment.id})" style="animation-delay: ${Math.random() * 0.5}s;">
            <div class="flex justify-between items-start mb-4">
                <div class="flex items-center">
...
function filterProjects() {
    ...
    if (budgetMin) {
        filtered = filtered.filter(p => (p.required_budget || 0) >= parseInt(budgetMin));
    }
    
    if (budgetMax) {
        filtered = filtered.filter(p => (p.required_budget || 0) <= parseInt(budgetMax));
    }
                        ...
                        ${relatedProjects.map(project => `
                            <div class="border border-gray-200 p-4 rounded-lg">
                                ...
                                <div class="flex justify-between text-xs text-gray-500">
                                    <span>Budget: $${(project.required_budget || 0).toLocaleString()}</span>
...
function showProjectDetail(projectId) {
    ...
    const totalFunding = relatedInvestments.reduce((sum, inv) => sum + inv.amount, 0);
            const fundingPercentage = project.required_budget ? (totalFunding / project.required_budget * 100) : 0;
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-blue-50 p-4 rounded-lg">
                    ...
                    <div class="font-semibold">$${(project.required_budget || 0).toLocaleString()}</div>
                </div>
            ...
            <div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="flex justify-between text-sm text-gray-600 mt-2">
                        ...
                        <span>Objectif: $${(project.required_budget || 0).toLocaleString()}</span>
                    </div>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="border border-gray-200 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-xs text-gray-500">
                            <span>Budget total: $${(relatedProject.required_budget || 0).toLocaleString()}</span>
...
async function submitProblem(event) {
    ...
    const formData = {
        ...
        affected_population: parseInt(document.getElementById('problem-population').value) || 0
    };
...
async function submitProject(event) {
    ...
    const formData = {
        ...
        required_budget: parseInt(document.getElementById('project-budget').value) || 0,
        expected_roi: parseInt(document.getElementById('project-roi').value) || 0,
        implementation_timeline: document.getElementById('project-timeline').value,
        country: document.getElementById('project-country').value,
        target_beneficiaries: parseInt(document.getElementById('project-beneficiaries').value) || 0
    };
    ...
    try {
        ...
        console.error('Erreur:', error);
        let errorMessage = 'Erreur lors de l\'ajout du projet';
...
async function submitInvestment(event) {
    ...
    const formData = {
        ...
        investor_id: 1, // ID par défaut pour l'investisseur
        project_id: 1  // ID par défaut pour le projet
    };
    ...
    try {
        ...
        console.error('Erreur:', error);
        let errorMessage = 'Erreur lors de l\'ajout de l\'investissement';
...
function showLoading(show) {
    ...
    if (loading) {
        if (show) {
            ...
            // Utiliser l'animation de chargement avancée
            if (window.animationManager) {
                window.animationManager.showLoading(loading, 'pulse-spinner');
            }
        } else {
            if (window.animationManager) {
                window.animationManager.hideLoading(loading);
            }
...
function showNotification(message, type = 'info') {
    ...
    if (!notification) {
        ...
        notification.className = 'fixed top-20 right-4 z-50 max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden';
        ...
        notificationText.className = 'text-sm font-medium';
        
        const closeButton = document.createElement('button');
        closeButton.className = 'ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-600 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8 items-center justify-center';
        closeButton.innerHTML = '<i class="fas fa-times"></i>';
        closeButton.onclick = () => hideNotification();
        
        const header = document.createElement('div');
        header.className = 'flex items-start';
        header.appendChild(notificationText);
    ...
    if (notificationText) {
        ...
        notification.className = `fixed top-20 right-4 z-50 max-w-sm w-full shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden`;
        
        switch(type) {
            case 'success':
                notification.classList.add('bg-green-50', 'border-l-4', 'border-green-400');
                notificationText.className = 'text-sm font-medium text-green-800';
                break;
            case 'error':
                notification.classList.add('bg-red-50', 'border-l-4', 'border-red-400');
                notificationText.className = 'text-sm font-medium text-red-800';
                break;
            case 'warning':
                notification.classList.add('bg-yellow-50', 'border-l-4', 'border-yellow-400');
                notificationText.className = 'text-sm font-medium text-yellow-800';
                break;
            default:
                notification.classList.add('bg-blue-50', 'border-l-4', 'border-blue-400');
                notificationText.className = 'text-sm font-medium text-blue-800';
        }
...
function getProjectFundingPercentage(project) {
    ...
    const budget = project.required_budget || 0;
